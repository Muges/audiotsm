...
def generate_test_cases(cases):
    """For each tuple (data, max_length, *params) of an array, generate
    multiple test cases (buffer, *params), where buffer is a CBuffer containing
    ``data``."""
    for case in cases:
        for buffer in generate_cbuffers(case[0], case[1]):
            yield (buffer, *case[2:])
...
@pytest.mark.parametrize("in_buffer, add, out", generate_test_cases([
    ...
    ([[1, 2, 3], [4, 5, 6]], 3, [[0.25, -0.25], [0.5, -0.5]],
     [[1.25, 1.75, 3], [4.5, 4.5, 6]]),
    ([[1, 2, 3], [4, 5, 6]], 5, [[0.25, -0.25], [0.5, -0.5]],
     [[1.25, 1.75, 3], [4.5, 4.5, 6]]),
    ([[1, 2, 3], [4, 5, 6]], 3, [[0.25, 0, -0.25], [0.5, 0, -0.5]],
     [[1.25, 2, 2.75], [4.5, 5, 5.5]]),
    ([[1, 2, 3], [4, 5, 6]], 5, [[0.25, 0, -0.25], [0.5, 0, -0.5]],
     [[1.25, 2, 2.75], [4.5, 5, 5.5]]),
]))
...
@pytest.mark.parametrize("in_buffer, array, out", generate_test_cases([
    ...
    ([[1, 2, 3], [4, 5, 6]], 3, [], [[1, 2, 3], [4, 5, 6]]),
    ([[1, 2, 3], [4, 5, 6]], 5, [], [[1, 2, 3], [4, 5, 6]]),
    ([[1, 2, 3], [4, 5, 6]], 3, [0.5], [[2, 2, 3], [8, 5, 6]]),
    ([[1, 2, 3], [4, 5, 6]], 3, [0.5, 0.25], [[2, 8, 3], [8, 20, 6]]),
    ([[1, 2, 3], [4, 5, 6]], 3, [0.5, 0.25, 3], [[2, 8, 1], [8, 20, 2]]),
    ([[1, 2, 3], [4, 5, 6]], 5, [0.5], [[2, 2, 3], [8, 5, 6]]),
    ([[1, 2, 3], [4, 5, 6]], 5, [0.5, 0.25], [[2, 8, 3], [8, 20, 6]]),
    ([[1, 2, 3], [4, 5, 6]], 5, [0.5, 0.25, 3], [[2, 8, 1], [8, 20, 2]]),
]))
